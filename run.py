...
async def unban(ctx, *, member):
    ...
    for ban_entry in banned_users:
        ...
        if (user.name, user.discriminator) == (member_name, member_discr): #Only takes name and discrimnator, may be a downside sometimes.
...
async def help(ctx):
	author = ctx.message.author
	embed=discord.Embed(title="insomnia's command help", description="A list of bot commands for insomnia", color=0x7CFC00)
	...
	await ctx.send(f"**:white_check_mark: Check your DM's** ***{author.display_name}!***")
...
@client.command(helpinfo='Info about servers ShrekBot is in', aliases=['server', 'num', 'count'])
...
async def servers(ctx):
    ...
    await ctx.send(':white_check_mark: ***Top servers with insomnia:***')
    for x in servers[:5]:
    ...
    for x in client.guilds:
        y += x.member_count
    await ctx.send('**Total number of insomnia users:** **{}**!\n**Number of servers:** **{}**!'.format(y, len(client.guilds)))
